...
-- Configuration system - SEPARATE FOLDER: KasumiHubPVB
local CONFIG_FILE = "ZanlikuHubPVB/PlantsBrainrot_Config.json"
local SETTINGS_FILE = "ZanlikuHubPVB/PlantsBrainrot_Settings.json"
local defaultConfig = {
    selectedSeeds = {},
    selectedGears = {},
    equipInterval = 20,
...
local availableSeeds = {
    ...
    "Tomatrio Seed", "Shroombino Seed", "Mango Seed","King Lemone Seed", "Starfruit Seed", "Brussel Sprout Seed"
}
...
local MAX_RECONNECT_ATTEMPTS = 30  -- ✅ Increased to 30 attempts
local RECONNECT_INTERVAL = 6       -- ✅ 6 seconds between attempts (30 × 6 = 180 seconds = 3 minutes)
...
local function saveSettings()
    ...
    if not isfolder("ZanlikuHubPVB") then
        makefolder("ZanlikuHubPVB")
    end
...
local function getAvailableConfigs()
    ...
    pcall(function()
        if isfolder("ZanlikuHubPVB") then
            local files = listfiles("ZanlikuHubPVB")
            for _, file in pairs(files) do
                if file:match("%.json$") and not file:match("Settings") then
                    local fileName = file:match("ZanlikuHubPVB/(.+)%.json$") or file:match("([^/\\]+)%.json$")
...
local function saveConfig(customName)
    ...
    local fileName = customName and ("ZanlikuHubPVB/" .. customName .. ".json") or CONFIG_FILE
...
-- ✅ CRITICAL FIX: Load settings AND config BEFORE creating UI
loadSettings()
...
if currentSettings.autoLoadEnabled and currentSettings.autoLoadConfig ~= "None" then
    ...
    local filesToTry = {
        "ZanlikuiHubPVB/" .. configName .. ".json",
        configName .. ".json"
    }
...
Window = WindUI:CreateWindow({
    Title = "Zanliku Hub ( In Testing )",
    Icon = "settings",
    Author = "@Zanliku",
    Folder = "ZanlikuHubPVB",
...
local SeedAutoBuyTab = Window:Tab({
    Title = "Seed Auto Buy",
    Icon = "shopping-cart",
    Locked = false
})

local GearAutoBuyTab = Window:Tab({
    Title = "Gear Auto Buy",
    Icon = "shopping-cart",
    Locked = false
})
...
InformationTab:Paragraph({
    Title = "Zanliku Hub for Plants vs. Brainrot",
...
SeedAutoBuyTab:Divider()

seedDropdown = SeedAutoBuyTab:Dropdown({
    Title = "Select Seeds",
...
autoBuyToggleSeeds = SeedAutoBuyTab:Toggle({
    ...
    Callback = function(enabled)
        ...
        WindUI:Notify({
            Title = enabled and "Seed Auto Buy Started" or "Seed Auto Buy Stopped",
            Content = enabled and string.format("Now automatically purchasing %d seed types", #selectedSeeds)
                     or "Automatic seed purchasing has been disabled",
            Duration = enabled and 3 or 2
        })
    end
})

autoBuyAllSeedsToggle = SeedAutoBuyTab:Toggle({
    ...
    Callback = function(enabled)
        ...
        WindUI:Notify({
            Title = enabled and "Buying All Seeds" or "Stopped Buying All Seeds",
            Content = enabled and string.format("Now purchasing all %d seed types automatically", #availableSeeds)
                     or "Switched back to your selected seeds only",
            Duration = enabled and 3 or 2
        })
    end
})
...
GearAutoBuyTab:Divider()

gearDropdown = GearAutoBuyTab:Dropdown({
    Title = "Select Gears",
...
autoBuyToggleGears = GearAutoBuyTab:Toggle({
    ...
    Callback = function(enabled)
        ...
        WindUI:Notify({
            Title = enabled and "Gear Auto Buy Started" or "Gear Auto Buy Stopped",
            Content = enabled and string.format("Now automatically purchasing %d gear types", #selectedGears)
                     or "Automatic gear purchasing has been disabled",
            Duration = enabled and 3 or 2
        })
    end
})

autoBuyAllGearsToggle = GearAutoBuyTab:Toggle({
    ...
    Callback = function(enabled)
        ...
        WindUI:Notify({
            Title = enabled and "Buying ALL Gears" or "Stopped Buying All Gears",
            Content = enabled and string.format("Now purchasing all %d gear types automatically", #availableGears)
                     or "Switched back to your selected gears only",
            Duration = enabled and 3 or 2
        })
    end
})
...
autoReconnectToggle = SettingsTab:Toggle({
    ...
    Value = autoReconnectEnabled,  -- ← FIXED
    Callback = function(enabled)
...
local configName = "Zanliku-Hub-pvb"

ConfigNameInput = SettingsTab:Input({
    Title = "Configuration Name",
    Value = configName,
    Callback = function(value)
        configName = value or "Zanliku-Hub-pvb"
    end
})
...
SettingsTab:Button({
    ...
    Callback = function()
        ...
        local filesToTry = {"Zanliku Hub PVB/" .. name .. ".json", name .. ".json"}
        local loaded = false
        for _, file in pairs(filesToTry) do
            if file and isfile(file) then
                local success, result = pcall(function()
...
task.spawn(function()
    task.wait(1)
    WindUI:Notify({
        Title = " Zanliku Hub " .. SCRIPT_VERSION .. " Ready",
        Content = string.format("Welcome! %s\nSeeds: %d | Gears: %d | Interval: %ds\n\nPress G to toggle interface", 
            currentSettings.autoLoadEnabled and currentSettings.autoLoadConfig ~= "None" and ("Auto-loaded: " .. currentSettings.autoLoadConfig) or "Enjoy!",
            #selectedSeeds,
            #selectedGears,
            equipInterval),
        Duration = 8
